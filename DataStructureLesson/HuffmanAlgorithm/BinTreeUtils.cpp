...
void writeTree(BinTree* tree, BinTreeTable* table, __int32 seq, BinTree* par) {
...
/*顺序遍历获取动态二叉树(最大)深度*/
__int32 getBinTreeDepth(BinTree* tree) {
	if (tree == NULL) return 0;
	BinTreeTable* table = convertTreeToTable(tree);
	__int32 max = getBinTreeDepth(table);
	delete[] table;
	return max;
}

/*顺序遍历获取静态表二叉树(最大)深度*/
__int32 getBinTreeDepth(BinTreeTable* table) {
	if (table == NULL) return 0;
	__int32 s = getBinTreeNodeNum(table, 0);
	if (s < 3) return s;
	__int32 max = 0;
	for (int i = 0; i < s; i++) {
...
/*递归遍历设置动态二叉树深度*/
void setBinTreeDepth(BinTree* tree, __int32 dep) {
	if (tree == NULL) return;
...
/*递归遍历设置静态表二叉树深度*/
void setBinTreeDepth(BinTreeTable* table, __int32 seq, __int32 dep) {
...
__int32 compareTree(BinTree* tree1, BinTree* tree2) {
...
__int32 compareTree(BinTree* tree1, BinTreeTable* table2) {
	BinTreeTable* t1 = convertTreeToTable(tree1);
	__int32 r = compareTree(t1, table2);
	delete[] t1;
	return r;
}

/*比较两棵二叉树是否严格相同,相同返回1,不同返回-1,重载+3*/
__int32 compareTree(BinTreeTable* table1, BinTree* tree2) {
	BinTreeTable* t2 = convertTreeToTable(tree2);
	__int32 r = compareTree(table1, t2);
	delete[] t2;
	return r;
}

/*比较两棵二叉树是否严格相同,相同返回1,不同返回-1,重载+4*/
__int32 compareTree(BinTreeTable* table1, BinTreeTable* table2) {
	if (((__int32)table1 ^ (__int32)table2) == 0) return 1;
	__int32 s1 = getBinTreeNodeNum(table1, 0);
	__int32 s2 = getBinTreeNodeNum(table2, 0);
...
/*递归遍历并销毁动态二叉树*/
__int32 destoryBinTree(BinTree* tree) {
...
/*顺序遍历销毁静态表二叉树*/
__int32 destoryBinTree(BinTreeTable* &table) {
	delete[] table;
	table = NULL;
	return 0;
}