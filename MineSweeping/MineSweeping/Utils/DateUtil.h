...
char* createFormattedTime(char* buf, unsigned int, const char*, const char*);

/*可以重载的函数,至少传入时间参数,返回格式化字符串,默认格式为 yyyy.mm.dd hh:mm:ss
 *使用完成后应及时回收内存
 */
char* getFormattedTime(char* buf, unsigned int);
char* getFormattedTime(char* buf, unsigned int, const char*);
char* getFormattedTime(char* buf, unsigned int, const char*, const char*);
